      ...
      <table class="orders-table">
        ...
        <tbody>
          ...
          <tr v-for="order in filteredOrders" :key="order.orderId">
            ...
            <td class="actions">
              <router-link :to="`/app/orders/${order.orderId}`" class="btn btn-sm btn-outline">
                Chi tiết
...
const fetchOrders = async () => {
  ...
  try {
    ...
    // Backend trả thẳng array, không wrap trong .data
...
const cancelOrder = async (id) => {
  ...
  try {
    await orderApi.put(`/orders/${id}/cancel`);
    await fetchOrders();
  } catch (e) {
    alert(e.response?.data?.message || e.message);
  }
...
.search-input { flex: 1; padding: 8px 12px; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 13px; }
.filter-select { padding: 8px 12px; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 13px; }
.stats-row { display: flex; gap: 12px; margin-bottom: 20px; }
.stat-card { flex: 1; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 10px; padding: 14px 18px; display: flex; flex-direction: column; gap: 4px; }
...
.table-wrapper { background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; overflow: hidden; }
...
.btn { padding: 7px 14px; border-radius: 6px; border: none; cursor: pointer; font-size: 13px; font-weight: 600; text-decoration: none; display: inline-block; }
